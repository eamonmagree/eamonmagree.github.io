...
let muted = document.getElementsByClassName("mute")[0]; /* this is the mute button im putting in. this section of code defines which part of the html code the variables are assigned to */
...
/* function HTMLMediaElement.muted */
muted.addEventListener("click", muteMedia); /* adding this event listener makes the function occur upon clicking the button */

function muteMedia() {

  if (media.muted == true) { /* the media element 'muted' has a boolean value meaning it is true or false  */
      document.getElementById('media').muted = false; /* if the button is clicked when the media is muted the video will be unmuted - vice versa for down below */
  }
  else 
  {
      media.muted == false;
      document.getElementById('media').muted = true;
  }
